...
public class GraphLayout extends FigureChangeAdapter {
    ...
    private Hashtable<Figure, GraphNode> nodes = new Hashtable<Figure, GraphNode>();
    private Hashtable<ConnectionFigure, Double> edges = new Hashtable<ConnectionFigure, Double>();
    ...
    private double len(Figure edge) {
        return (edges.get(edge)).doubleValue() * LENGTH_FACTOR;
    }
...
class GraphNode {
    ...
    void update() {
        ...
        if (Math.abs(p.x - Math.round(x)) > 1
            ...
            x = p.x;
            y = p.y;
            // logger.debug(this+" has new coords: "+x+","+y+"\n");
        }
    }
}
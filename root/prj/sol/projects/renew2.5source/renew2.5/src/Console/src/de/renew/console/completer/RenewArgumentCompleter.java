...
public class RenewArgumentCompleter implements Completer {
     ...
     */
    public RenewArgumentCompleter(final ArgumentDelimiter delimiter,
                                  final Collection<Completer> completers) {
    ...
    public int complete(final String buffer, final int cursor,
        ...
        // if the buffer is null, just use the first completer
        if (argIndex < 0) {
        ...
        for (int i = 0; isStrict() && (i < argIndex); i++) {
            Completer sub = completers.get(i >= completers.size()
                                           ? (completers.size() - 1) : i);
        ...
        int ret = completer.complete(list.getCursorArgument(), argpos,
                                     candidates);
        ...
        if ((buffer != null) && (cursor != buffer.length())
                    && delim.isDelimiter(buffer, cursor)) {
    ...
    public abstract static class AbstractArgumentDelimiter
        ...
        public ArgumentList delimit(final CharSequence buffer, final int cursor) {
            ...
            return new ArgumentList(args.toArray(new String[args.size()]),
    ...
    public static class ArgumentList {
         ...
         */
        public ArgumentList(final String[] arguments,
                            final int cursorArgumentIndex,
                            final int argumentPosition, final int bufferPosition) {

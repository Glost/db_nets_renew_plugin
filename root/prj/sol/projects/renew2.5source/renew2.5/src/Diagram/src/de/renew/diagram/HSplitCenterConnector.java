...
public class HSplitCenterConnector extends AbstractConnector {
    public HSplitCenterConnector() { // only used for Storable implementation
    }

    public HSplitCenterConnector(Figure owner) {
        super(owner);
    }
    ...
    protected Point chop(Figure target, Rectangle source) {
        ...
        if (source.x > r.x + r.width / 2) {
            horizontal += SplitDecoration.getDefaultSize();
        }
    ...
    public Rectangle displayBox() {
        ...
        return new Rectangle(r.x, (r.y + r.height / 2), 0, 0);
    }
}